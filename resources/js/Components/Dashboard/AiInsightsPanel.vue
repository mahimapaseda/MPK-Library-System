    ...
    <div class="sm:col-span-2 xl:col-span-4 glass-card rounded-3xl overflow-hidden flex flex-col h-full">
        ...
        <div class="p-4 sm:p-6 lg:p-8 grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6 flex-1 overflow-y-auto min-h-0">
            ...
            <div class="xl:col-span-1 p-5 rounded-2xl bg-white/50 dark:bg-slate-900/40 border border-white/20 dark:border-slate-700/30">
                <div class="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest">Prediction Snapshot</div>
            ...
            <div class="xl:col-span-2 p-5 rounded-2xl bg-white/50 dark:bg-slate-900/40 border border-white/20 dark:border-slate-700/30">
                <div class="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-3">Automated Alerts</div>
                <div class="space-y-3 mb-5">
                    <div v-for="(alert, idx) in insights.alerts" :key="`alert-${idx}`"
                        ...
                        <div class="text-xs text-slate-700 dark:text-slate-400 mt-1">{{ alert.detail }}</div>
                    </div>
                </div>

                <div class="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-3">Recommended Actions</div>
                <div class="space-y-3">
                    <div v-for="(item, idx) in insights.recommendations" :key="idx"
                        ...
                        <div class="text-xs text-slate-700 dark:text-slate-400 mt-1">{{ item.detail }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="xl:col-span-3 p-5 rounded-2xl bg-white/50 dark:bg-slate-900/40 border border-white/20 dark:border-slate-700/30">
                <div class="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-3">High-Demand Books (Low Stock)</div>
                <div v-if="insights.high_demand_books.length === 0" class="text-sm text-slate-700 dark:text-slate-400">No critical stock-outs detected right now.</div>
                <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div v-for="book in insights.high_demand_books" :key="book.id" class="p-3 rounded-xl border border-white/20 dark:border-slate-700/30">
                        ...
                        <div class="text-xs text-slate-700 dark:text-slate-400 mt-1">{{ book.author }}</div>
                    </div>
                </div>
            </div>
        </div>
