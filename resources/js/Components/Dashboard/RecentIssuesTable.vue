            ...
            <table class="w-full min-w-160">
                <thead>
                    <tr class="bg-white/5 dark:bg-slate-800/20 sticky top-0 z-10">
                        <th class="px-4 sm:px-6 lg:px-8 py-4 text-left text-xs font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest leading-none">Book</th>
                        <th class="px-4 sm:px-6 lg:px-8 py-4 text-left text-xs font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest leading-none">Member</th>
                        <th class="px-4 sm:px-6 lg:px-8 py-4 text-left text-xs font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest leading-none">Due Date</th>
                        <th class="px-4 sm:px-6 lg:px-8 py-4 text-right text-xs font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest leading-none">Status</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-white/5 dark:divide-slate-800/30">
                    <tr v-for="issue in issues" :key="issue.id" class="group hover:bg-white/40 dark:hover:bg-white/5 transition-colors">
                        <td class="px-4 sm:px-6 lg:px-8 py-5">
                            ...
                            <div class="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-tighter mt-0.5">Ref: #{{ issue.id }}</div>
                        </td>
                        <td class="px-4 sm:px-6 lg:px-8 py-5">
                            <div class="text-sm font-bold text-slate-700 dark:text-slate-300">{{ issue.member.name }}</div>
                            <div class="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-tighter mt-0.5">{{ issue.member.member_id }}</div>
                        </td>
                        <td class="px-4 sm:px-6 lg:px-8 py-5">
                            <div class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ new Date(issue.due_date).toLocaleDateString() }}</div>
                        </td>
